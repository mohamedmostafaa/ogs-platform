...
- [ ] `OGS-026` Provision Arcjet site; per-env `ARCJET_KEY`; add to `.env.example` + `turbo.json` + Vercel — @security-engineer + @devops-engineer — SECURITY.md §6
- [ ] `OGS-027` Scaffold `packages/security/src/arcjet/index.ts` with 5 canonical rule sets — @security-engineer — SECURITY.md §6.2
- [ ] `OGS-028` Add `apps/web-*/src/proxy.ts` mounting Arcjet on every app (publicShield baseline) — @security-engineer + @devops-engineer — SECURITY.md §6.3
...
`pnpm dev` brings up all 8 apps + Inngest dev + Prisma Studio. CI is green on `main`. `pnpm version-check` exits 0. Arcjet console shows denied requests on preview. Sentry receives a test error from each app. `pr-discipline.yml`, `verify-no-dead-names.yml`, `stack-currency.yml`, gitleaks all reject a synthetic bad PR.
...
- [ ] `OGS-030` `PAGINATION` constants — @devops-engineer — §11.1
- [ ] `OGS-031` Shared enums (Locale, AppName) — @devops-engineer — §4

### @ogs/db — Prisma 7 (multi-file schema + driver adapter + ESM)

- [ ] `OGS-040` Workspace setup; install `prisma@^7`, `@prisma/client@^7`, `@prisma/adapter-pg@^7`, `dotenv@^17` — @database-engineer — §29.5
- [ ] `OGS-041` `prisma/schema/_datasource.prisma` (provider `prisma-client`, no `url`) — @database-engineer — §5.4
- [ ] `OGS-042` `prisma/schema/auth.prisma` (Better Auth tables) — @database-engineer — §5.5
- [ ] `OGS-043` `prisma/schema/tenancy.prisma` (Tenant, Membership) — @database-engineer — §5.6
- [ ] `OGS-044` `prisma/schema/worker.prisma` — @database-engineer — §5.7
- [ ] `OGS-045` `prisma/schema/company.prisma` (Company, Enterprise) — @database-engineer — §5.8
- [ ] `OGS-046` `prisma/schema/careers.prisma` — @database-engineer — §5.9
- [ ] `OGS-047` `prisma/schema/academy.prisma` — @database-engineer — §5.10
- [ ] `OGS-048` `prisma/schema/skillpass.prisma` — @database-engineer — §5.11
- [ ] `OGS-049` `prisma/schema/payments.prisma` — @database-engineer — §5.12
- [ ] `OGS-050` `prisma/schema/secrets.prisma` (SecretCredential, includes ARCJET_KEY-class types) — @database-engineer — §5.13
- [ ] `OGS-051` `prisma/schema/ai.prisma` (AIInteraction, PromptVersion, EvalRun, KnowledgeDocument, EmbeddingChunk) — @database-engineer — §5.14
- [ ] `OGS-052` `prisma/schema/notifications.prisma` (Document, Notification, NotificationPreference) — @database-engineer — §5.15
- [ ] `OGS-053` `prisma/schema/audit.prisma` (AuditLog, WebhookEvent) — @database-engineer — §5.16
- [ ] `OGS-054` `prisma/schema/platform.prisma` (FeatureFlag, AppSettings) — @database-engineer — §5.17
- [ ] `OGS-055` First `prisma migrate dev` against Neon dev — @database-engineer — §5.18
- [ ] `OGS-056` `packages/db/src/extensions/soft-delete.ts` — @database-engineer — §16.2
- [ ] `OGS-057` `packages/db/src/extensions/tenant-scope.ts` (AsyncLocalStorage) — @database-engineer — §16.3
- [ ] `OGS-058` `packages/db/src/extensions/audit.ts` — @database-engineer — §16.4
- [ ] `OGS-059` Composed Prisma client `packages/db/src/index.ts` — @database-engineer — §16.1
- [ ] `OGS-060` `packages/db/src/run-with-actor.ts` envelope — @database-engineer — §16.5
- [ ] `OGS-061` `prisma/seed.ts` minimal seed — @database-engineer — §5
- [ ] `OGS-062` `packages/db/prisma.config.ts` with `@prisma/adapter-pg` + `import "dotenv/config"` (Prisma 7) — @database-engineer — §5.4.1
- [ ] `OGS-063` `packages/db/src/client.ts` instantiates `new PrismaClient({ adapter: new PrismaPg(...) })` — @database-engineer — §5.4.3
- [ ] `OGS-064` `packages/db/package.json` set `"type": "module"` (ESM mandatory in Prisma 7) — @database-engineer — §5.4.4

### @ogs/auth — Better Auth 1.6 + @better-auth/oauth-provider

- [ ] `OGS-070` Workspace setup; install `better-auth@^1.6`, `@better-auth/oauth-provider@^0.1`, `nanoid@^5` — @auth-engineer — §29.6
- [ ] `OGS-071` `client-config.ts` per-app OIDC client factory — @auth-engineer — §6.4
- [ ] `OGS-072` `guards.ts` (`requireAuth`, `requireRole`, `requireTenant`, `requireFeature`, `trpcRequireAuth`) — @auth-engineer — §6.8
- [ ] `OGS-073` Stub `server/provisioning.ts` — @auth-engineer — §6.6

### @ogs/api — tRPC v11 + superjson

- [ ] `OGS-080` Workspace setup; install `@trpc/server@^11.17`, `@trpc/client@^11.17`, `@tanstack/react-query@^5.100`, `superjson@^2` — @api-engineer — §29.7
- [ ] `OGS-081` `trpc.ts` (context + procedure hierarchy) — @api-engineer — §7.1
- [ ] `OGS-082` `query-client.ts` — @api-engineer — §7.2
- [ ] `OGS-083` `server-helpers.tsx` (prefetch + HydrateClient) — @api-engineer — §7.3
- [ ] `OGS-084` `client.tsx` TRPCReactProvider — @api-engineer — §7.4
- [ ] `OGS-085` Empty `root.ts` with `appRouter` + `AppRouter` type — @api-engineer — §7.6

### @ogs/ui — full shadcn primitive set + EntityX + theme + Avatar

- [ ] `OGS-090` Workspace setup; shadcn init — @ui-engineer — §29.8
- [ ] `OGS-091` Install full shadcn primitive set (~50) in one batch — @ui-engineer — §9.1
- [ ] `OGS-092` `lib/cn.ts` — @ui-engineer — §9.8
- [ ] `OGS-093` `theme/tokens.css` (light + dark) — @ui-engineer — §3.5.1
- [ ] `OGS-094` `theme/provider.tsx` (OgsThemeProvider) — @ui-engineer — §3.5.2
- [ ] `OGS-095` `theme/brand.ts` per-tenant resolver — @ui-engineer — §3.5.3
- [ ] `OGS-096` `components/theme-toggle.tsx` — @ui-engineer — §3.5.4
- [ ] `OGS-097` `data-display/entity.tsx` (EntityHeader/Container/Search/Pagination/Loading/Error/Empty/List/Item) — @ui-engineer — §9.2
- [ ] `OGS-098` `data-display/data-table.tsx` (TanStack Table) — @ui-engineer — §9.3
- [ ] `OGS-099` `components/avatar.tsx` (UserAvatar + AgentAvatar; no DiceBear) — @ui-engineer — §9.4
- [ ] `OGS-100` Bundle 12 neutral geometric SVGs in `src/assets/agent-avatars/` — @ui-engineer — §9.4
- [ ] `OGS-101` `use-error-modal.tsx` — @ui-engineer — §9.5
- [ ] `OGS-102` `forms/use-entity-search.ts` (500 ms debounce) — @ui-engineer — §9.6
- [ ] `OGS-103` `use-confirm.tsx` (promise-based) — @ui-engineer — §10.3
- [ ] `OGS-104` Turbo task to copy `assets/agent-avatars/` to every app's `public/` — @ui-engineer — §9.4
...
- [ ] `OGS-150` `packages/email/src/transport.ts` (Nodemailer 8 + SMTP, pooled) — @notifications-engineer — §18.3
- [ ] `OGS-151` `magic-otp.tsx` React Email template — @notifications-engineer — §18.4
- [ ] `OGS-152` `render.ts` dispatcher — @notifications-engineer — §18.4
