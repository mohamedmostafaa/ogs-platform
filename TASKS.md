...
- [ ] `OGS-001` Create the GitHub repo and push the initial commit — @engineering-lead — §29.2
- [ ] `OGS-002` Branch protection on `main` — @engineering-lead — §23.10
- [ ] `OGS-003` CODEOWNERS in repo — @engineering-lead — §23.10
- [ ] `OGS-021` CI workflow `ci.yml` (lint, typecheck, test, build) — @devops-engineer — §23.10
- [ ] `OGS-022` CI workflow `pr-discipline.yml` (TASKS update + skills section + linked task + blueprint ref) — @devops-engineer — AGENTS.md §9
- [ ] `OGS-023` gitleaks pre-commit + CI — @security-engineer — SECURITY.md §0 rule 5
- [ ] `OGS-032` CI workflow `stack-currency.yml` (`pnpm version-check` nightly + on PR) — @devops-engineer + @security-engineer — blueprint §3.2.1
- [ ] `OGS-033` CI workflow `verify-no-dead-names.yml` (block legacy major patterns) — @devops-engineer + @architecture-reviewer — blueprint §3.1.1

### Node, pnpm, workspace, tooling

- [ ] `OGS-004` `.nvmrc` (Node 24) — @devops-engineer — §29.1
- [ ] `OGS-005` Root `package.json` with `"type": "module"`, engines node ≥ 24, pnpm ≥ 11 — @devops-engineer — §4.8
- [ ] `OGS-006` `pnpm-workspace.yaml` — @devops-engineer — §4.7
- [ ] `OGS-007` `turbo.json` (Turborepo 2.9) — @devops-engineer — §4.6
- [ ] `OGS-008` `tsconfig.base.json` (TypeScript 6) — @devops-engineer — §4.12
- [ ] `OGS-009` ESLint 10 flat config + Prettier 3.8 — @devops-engineer — §4.10–§4.11
- [ ] `OGS-010` `mprocs.yaml` — @devops-engineer — §4.9
- [ ] `OGS-011` `tooling/{tsconfig,eslint-config,tailwind-config}` shared workspaces — @devops-engineer — §29.4
- [ ] `OGS-012` `tooling/scripts/bootstrap.sh` (idempotent) — @devops-engineer — §29.15
...
- [ ] `OGS-030` Author `tooling/scripts/version-check.ts`; wire `pnpm version-check` script — @devops-engineer + @security-engineer — blueprint §3.2.1
- [ ] `OGS-031` Configure renovate.json for weekly minor + patch upgrades within locked floors — @devops-engineer — blueprint §3.2.3
...
- [ ] `OGS-024` ADR scaffolding (`0000-template.md`, `INDEX.md`) — @docs-writer — §28.3
