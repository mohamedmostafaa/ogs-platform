...
const LOCKED: readonly Locked[] = [
  ...
  // Jobs
  { name: "inngest", floor: "4.4.0", owner: "@inngest-engineer" },
...
function fetchLatest(name: string): string {
  try {
    return execSync(`npm view ${name} version --silent`, {
      encoding: "utf8",
...
if (wantJson) {
  ...
  console.log(
    `${"PACKAGE".padEnd(40)} ${"LOCKED".padEnd(12)} ${"LATEST".padEnd(12)} ${"STATUS".padEnd(8)} ${"DELTA".padEnd(8)} OWNER`,
  );
  ...
  console.log(
    `Summary: ${counts.GREEN} green · ${counts.YELLOW} yellow · ${counts.ORANGE} orange · ${counts.RED} red · ${counts.UNKNOWN} unknown`,
  );
}
